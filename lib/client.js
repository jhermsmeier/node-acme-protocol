...
Client.prototype = {
  ...
  configure: function(/* [path,] callback */) {
    ...
    this._clientRequest( path, function( error, response, body ) {
      self.directory = body
  ...
  getNonce: function( callback ) {
    ...
    this._clientRequest({
      method: 'HEAD',
      url: this.options.directoryPath,
   ...
   * @return {Object}
   */
  getJWSPayload: function( payload, header, protectedFields ) {
  ...
  register: function( contact, callback ) {
    ...
    var payload = this.getJWSPayload( registration, {
      alg: 'RS256',
    ...
    this._clientRequest({
      ...
      self.registrationUrl = response.headers[ 'location' ]
  ...
  updateRegistration: function( registration, callback ) {
    ...
    var self = this
    var location = URL.parse( this.registrationUrl ).pathname

    registration.key = JSONWebKey.fromPEM( this.options.publicKey )

    debug( 'registration:location', location )
  ...
  newAuthorization: function( authorization, callback ) {
    ...
    var payload = this.getJWSPayload( authorization, {
      alg: 'RS256',
      jwk: JSONWebKey.fromPEM( this.options.publicKey ),
      nonce: this._replayNonce,
      typ: 'JWS',
  ...
  newApplication: function( callback ) {
    ...
    var body = this.getJWSPayload( payload, {
      alg: 'RS256',
      jwk: JSONWebKey.fromPEM( this.options.publicKey ),
      nonce: this._replayNonce,
      typ: 'JWS',
