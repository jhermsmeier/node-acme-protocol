...
Client.prototype = {
  ...
  _clientRequest: function( options, callback ) {
    ...
    // Automatically configure the directory
    if( !this.isConfigured && options.url !== this.directoryUrl ) {
      return this.configure( function( error ) {
        if( error ) return void callback.call( self, error )
        self._clientRequest( options, callback )
      })
    }
    ...
    return this._request( options, function( error, response, body ) {
      ...
      // Automatically replenish the nonce
      if( !self._nonces.length && options.url !== self.directoryUrl ) {
        return self.getNonce( function( error ) {
          if( error ) return void callback.call( self, error )
          self._clientRequest( options, callback )
        })
      }
  ...
  configure: function(/* [path,] callback */) {
    ...
    this._clientRequest({ url: path }, function( error, response, body ) {
      self.directory = body
  ...
  getRegistration: function( callback ) {
    this.updateRegistration({ resource: ACME.REGISTRATION }, callback )
  },
  ...
  updateRegistration: function( registration, callback ) {
    ...
    if( !this.hasCapability( ACME.REGISTRATION ) ) {
      return void callback.call( this, new Error( 'Registration resource not supported' ) )
    }

    registration = registration || this.registartion
    registration.key = this.publicKey

    debug( 'registration:location', this.registrationUrl )
  ...
  newAuthorization: function( authorization, callback ) {
    ...
    if( !this.hasCapability( ACME.NEW_AUTHORIZATION ) ) {
  ...
  newApplication: function( callback ) {

    if( !this.hasCapability( ACME.NEW_APPLICATION ) ) {
