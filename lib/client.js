...
Client.getResponseError = function( error, response, body ) {
  ...
  if( response.headers['content-type'] === 'application/problem+json' ) {
    ...
    error.message = body.detail || ACME[ 'E_' + errorType ]
...
Client.prototype = {
  ...
  getNonce: function( callback ) {
    ...
    return this._request.head( this.options.directoryPath, function( error, response, body ) {
  ...
  register: function( contact, callback ) {
    ...
    if( !this.hasCapability( ACME.NEW_REGISTRATION ) ) {
      return callback.call( this, new Error( 'New registrations not supported' ) )
    }

    if( !this._replayNonce ) {
      return callback.call( this, new Error( 'Empty replay nonce' ) )
    }
    ...
    return this._request.post( '/acme/new-reg', {
      body: body,
